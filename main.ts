...
class Account {
  ...
  constructor(public readonly accountNumber: string, public readonly owner: string) {
    this.balance = 0;
  }
...
async function accountMenu(account: Account) {
  while (true) {
    const { action } = await inquirer.prompt([
      {
        type: 'list',
  ...
  const { amount } = await inquirer.prompt([
    {
      ...
      validate: (value:any) => !isNaN(Number(value)) && Number(value) > 0 || 'Amount must be a positive number'
    }
  ]);
...
async function checkBalance(account: Account) {
